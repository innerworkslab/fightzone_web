...
const adminId = modalStore.initialValues.id as number;
const username = modalStore.initialValues.name as string;

const { updateAdmin, loading: isSubmitting } = AdminsServices.useAdminActions();
...
const submitForm = handleSubmit(async (values) => {
    try {
        ...
        const response = await updateAdmin(adminId, payload as AdminsPayload);
    ...
    <form @submit.prevent="submitForm" class="space-y-6">
        <div class="flex items-center gap-3 mb-2 p-3 bg-primary/5 border border-primary/10 rounded-lg">
            ...
            <div>
                ...
                <p class="text-[10px] text-muted-foreground uppercase font-bold">Update system access for UID: {{
                    adminId }}</p>
            </div>
        </div>
