...
if (!function_exists('UploadFileToServer')) {
    function UploadFileToServer(Request $request, string $file_name, string $path, string $disk='public'): ? array
    {
        try{
            $file = $request->file($file_name);
            $file_path = $file->store($path, $disk);
...
if (!function_exists('MyanmarTime')) {
    function MyanmarTime($timeStr)
    {
        $time = Carbon::parse($timeStr);//string to time
        $time = Carbon::parse($timeStr); // string to time
        $time->addHours(6);
        $time->addMinutes(30);
